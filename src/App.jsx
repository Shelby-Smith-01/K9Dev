...
import mqtt from "mqtt/dist/mqtt.min.js"; // browser build
...
function ConnectionPanel({ conn, setConn, onConnect, onDisconnect, status, msgs }) {
  return (
    <div style={{padding:12, background:'rgba(255,255,255,0.9)', border:'1px solid #e5e7eb', borderRadius:16, boxShadow:'0 4px 16px rgba(0,0,0,.08)', maxWidth:420}}>
      <div style={{fontSize:12, fontWeight:600, marginBottom:6}}>MQTT WebSocket</div>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8}}>
        <label style={{fontSize:12}}>Host<input style={{width:'100%'}} value={conn.host} onChange={(e)=>setConn({...conn, host:e.target.value})}/></label>
        <label style={{fontSize:12}}>Port<input style={{width:'100%'}} value={conn.port} onChange={(e)=>setConn({...conn, port:Number(e.target.value)})}/></label>
        <label style={{fontSize:12}}>Path<input style={{width:'100%'}} value={conn.path} onChange={(e)=>setConn({...conn, path:e.target.value})}/></label>
        <label style={{fontSize:12, display:'flex', alignItems:'center', gap:6}}>SSL<input type="checkbox" checked={conn.ssl} onChange={(e)=>setConn({...conn, ssl:e.target.checked})}/></label>
      </div>
      <label style={{fontSize:12, display:'block', marginTop:8}}>Topic<input style={{width:'100%'}} value={conn.topic} onChange={(e)=>setConn({...conn, topic:e.target.value})}/></label>
      <div style={{display:'flex', alignItems:'center', gap:8, marginTop:8, fontSize:13}}>
        ...
        <span style={{display:'inline-flex', alignItems:'center', gap:8, marginLeft:8}}>
          <span style={{width:10, height:10, borderRadius:'50%', background: status==='connected'?'#22c55e': status==='error'?'#ef4444':'#d1d5db'}}></span>
...
function useMQTT(conn, onMessage) {
  ...
  const connect = () => {
    ...
    let c;
    try {
      c = mqtt.connect(url, { connectTimeout: 6000, clientId: `web-${Math.random().toString(16).slice(2)}` });
    } catch (e) {
      setStatus("error");
    ...
    c.on("connect", () => {
      ...
      c.subscribe(conn.topic, { qos: 0 }, (err) => {
        if (err) { setStatus("error"); console.error("subscribe error", err); }
      });
    });
    c.on("reconnect", () => setStatus("reconnecting"));
    c.on("error", (err) => { setStatus("error"); console.error("mqtt error", err); });
  ...
  return { status, msgs, connect, disconnect };
}

function Recenter({ lat, lon }) {
  const map = useMap();
  useEffect(() => { if (Number.isFinite(lat) && Number.isFinite(lon)) map.setView([lat, lon]); }, [lat, lon]);
  return null;
}
...
export default function App() {
  ...
  const { status, msgs, connect, disconnect } = useMQTT(conn, (msg) => {
  ...
  const stopTrack = async () => {
    ...
    // Fetch simple weather and elevation
